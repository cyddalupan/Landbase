...
class Applicants  extends Admin_Controller {
	...
	public function all()
	{   
		//get current page, use for cache
		$AllAppPage = 1;
		if(isset($_GET['page'])){
			$AllAppPage = $_GET['page'];
		}
        ...
        if ( isset( $_GET['search'] ) ) {
        	...
        	if ( ! $applicants = $this->cache->file->get('applicants'.$searchjson.$AllAppPage))
			{
	        	$applicants = ( new m_applicant )->searchApplicants( Pagination::getPerPage(), Pagination::getRecordCursor() );
        		$this->cache->file->save('applicants'.$searchjson.$AllAppPage, $applicants, 300);
			}

        	if ( ! $applicantsCount = $this->cache->file->get('applicantsCount'.$searchjson.$AllAppPage))
			{
        		$applicantsCount = ( new m_applicant )->searchApplicantsCount();
        		$this->cache->file->save('applicantsCount'.$searchjson.$AllAppPage, $applicantsCount, 300);
			}
        } else {
        	$sortjson = json_encode($sort);
        	$optionsjson = json_encode($options);


        	if ( ! $applicants = $this->cache->file->get('applicants'.$optionsjson.$AllAppPage.$sortjson))
			{
	        	$applicants = ( new m_applicant )->getApplicants( $options, Pagination::getPerPage(), Pagination::getRecordCursor(), $sort );
        		$this->cache->file->save('applicants'.$optionsjson.$AllAppPage.$sortjson,$applicants, 300);
			}

        	if ( ! $applicantsCount = $this->cache->file->get('applicantsCount'.$optionsjson.$AllAppPage.$sortjson))
			{
	        	$applicantsCount = ( new m_applicant )->getApplicantsCount( $options );
	        	$this->cache->file->save('applicantsCount'.$optionsjson.$AllAppPage.$sortjson,$applicantsCount, 300);
			}
		...
		if ( ! $employers = $this->cache->file->get('employers'))
		{
			$this->load->model('m_employer');
			$employers = ( new m_employer )->all();
			$this->cache->file->save('employers', $employers, 1300);
		}

		if ( ! $categories = $this->cache->file->get('categories'))
		{
			$this->load->model( 'm_position');
        	$categories = ( new m_position )->getActivePositionsGroupByCategory();
			$this->cache->file->save('categories', $categories, 1400);
		}

		if ( ! $countries = $this->cache->file->get('countries'))
		{
			$this->load->model( 'm_country');
			$countries  = ( new m_country )->getCountries();
			$this->cache->file->save('countries', $countries, 1500);
		}	
        ...
        foreach ($applicants as $applicant_key => $applicant) {
        	...
        	if ( ! $applicants[$applicant_key]["employer_names"] = $this->cache->file->get('applicants'.$applicant['applicant_id'].'|'.$AllAppPage))
			{
	        	$applicants[$applicant_key]["employer_names"] = ( new m_applicant )->cyd_get_multiple_employer( $applicant['applicant_id'] );
				$this->cache->file->save('applicants'.$applicant['applicant_id'].'|'.$AllAppPage, $applicants[$applicant_key]["employer_names"], 300);
			}
		...
		foreach ($applicants as $key => $applicantValue) {	
			if ( ! $workExp = $this->cache->file->get('workExp'.'|'.$applicantValue['applicant_id']))
			{
				$workExp = ( new m_applicant )->getApplicantWorkExperiences( $applicantValue['applicant_id'] );
				$this->cache->file->save('workExp'.'|'.$applicantValue['applicant_id'], $workExp, 300);
			}
			$appTotalExp[$applicantValue['applicant_id']] = 0;
			
			//work experience total
			foreach ($workExp as $key => $ExpValue) {
				if ($ExpValue['experience_to']=='' || $ExpValue['experience_to']=='up to present' ){
				$dateto='2015-10-15';
				}
				else {
				$dateto=$ExpValue['experience_to'];
				}
				
				$date1 = date_parse($ExpValue['experience_from']);
				$date2 = date_parse($dateto);
		...
		if ( ! $subpositions = $this->cache->file->get('subpositions'))
		{
			$subpositions = ( new m_applicant )->cyd_get_all_sub_position();
			$this->cache->file->save('subpositions', $subpositions, 1400);
		}

		if ( ! $applicants_raw = $this->cache->file->get('applicants_raw'))
		{
			$applicants_raw = ( new m_applicant )->cyd_get_applicants_raw();
			$this->cache->file->save('applicants_raw', $applicants_raw, 2000);
		}
		
		if ( ! $applicant_certificate_raw = $this->cache->file->get('applicant_certificate_raw'))
		{
			$applicant_certificate_raw = ( new m_applicant )->cyd_get_applicant_certificate_raw();
			$this->cache->file->save('applicant_certificate_raw', $applicant_certificate_raw, 1400);
		}

		if ( ! $recruitmentAgents = $this->cache->file->get('recruitmentAgents'))
		{
			$this->load->model( 'm_recruitment_agent');	
			$recruitmentAgents = ( new m_recruitment_agent )->getRecruitmentAgents();
			$this->cache->file->save('recruitmentAgents', $recruitmentAgents, 1400);
		}
		...
		$this->setVariables([
				...
				'recruitmentAgents'	=> $recruitmentAgents,
  ...
  $this->scripts = [
	...
	{   
        ...
        $options     = [];
        $sort        = ['requirement_contract', 'DESC'];
