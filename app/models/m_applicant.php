...
class m_applicant extends MY_Model {
 	/* Constants
	-------------------------------*/
    const RESERVED_DAYS_EXPIRATION = 14; //7-days, update also on the MySQL trigger
    ...
    public $fileTypes = [   
		...
		'Doc 8'              => 'Docs 8',
        'Other'              => 'Other',
		'Agency File 1'      => 'Agency File 1',
		'Agency File 2'      => 'Agency File 2',
		'Agency File 3'      => 'Agency File 3',
		'Agency File 4'      => 'Agency File 4',
		'Agency File 5'      => 'Agency File 5',
		'Agency File 6'      => 'Agency File 6',
		'Agency File 7'      => 'Agency File 7',
		'Agency File 8'      => 'Agency File 8'
...
public function searchApplicants()
	{
		...
		if ( $search['recruitmentAgent'] > 0 ) {
			$this->db->where([
				'agent_id' => $search['recruitmentAgent']
			]);
		}
		...
		if ( $search['position'] > 0 ) {
			...
			//$this->db->where_in('a.applicant_id',$all_apid_sub_pos);
		}
    ...
    public function getExpiredVisaApplicants( $options = [], $limit = 0, $offset = 0, $sort = [ 'requirement_visa_expiration', 'ASC' ] )
    {
        ...
        $this->db->from( 'applicant_view' )
            ->where([
                'requirement_visa'               => 1,
				'requirement_visa_expiration !='  => '0000-00-00',
    ...
    public function getExpiredPassportsApplicants( $options = [], $limit = 0, $offset = 0, $sort = [ 'passport_expiration', 'ASC' ] )
    {
        ...
        $this->db->from( 'applicant_view' )
            ...
            ->where([
                'passport_number !='     => '',
                'passport_expiration <=' => date( 'Y-m-d', time() + 60*60*24*30*5 ),
                'passport_expiration >'  => DATE_EMPTY,
            ]);

    		$notinclude = array(1,7,9,13);
            $this->db->where_not_in('applicant_status',$notinclude);
...
*/
			...
			$this->db->from( 'applicant_log' )
				->where( "`log_created` BETWEEN '".$dateFrom."' AND '".$dateTo."' ", null, false)
				->where([
					'log_status'       => $this->status['For Booking'],
				]);
	...
	{	
		...
		$applicantData = [
			...
			'personalAbilities'			   => $basic['personalAbilities'],
			...
			for ( $i = 0; $i < count( $experiences['company'] ); $i++ ) {
				...
				$experiencesData[] = [
					...
					'experience_company'	=> $experiences['company'][$i],
					 'nationality' 		 	=> $experiences['nationality'][$i],
					'experience_position'	=> $experiences['position'][$i],
					'NoFamilyMembers' 		=> $experiences['NoFamilyMembers'][$i],
					'typeOfResidence' 		=> $experiences['typeOfResidence'][$i],
					...
					'reasonOfLeaving'       => $experiences['reasonOfLeaving'][$i],
					'experience_years'      => '',
  ...
  //       $visaData = $this->rawApplicantVisa([ 'visa_applicant' => $applicantId]);
        ...
        $this->addLog('Applicant has been registered.', $applicantId, 0, $this->status['For Review'], date( 'Y-m-d', time() ) );
	...
	public function updateApplicantProfile( $applicantId )
	{
		...
		$resultfile = fopen("testoutput.txt", "w");
		fwrite($resultfile, 'add work experience start');
		...
		if ( isset( $newWorkExperiences['company'] ) ) {
			for ( $i = 0; $i < count( $newWorkExperiences['company'] ); $i++ ) {				
				...
				if(!isset($newWorkExperiences['reasonOfLeaving'][$i])){
					$newWorkExperiences['reasonOfLeaving'][$i] = '';
				}
			...
			fwrite($resultfile, PHP_EOL.'added var');
			...
			fwrite($resultfile, PHP_EOL.json_encode($workExperiencesData));

				$experienceInserted = $this->db->insert_batch('applicant_experiences', $workExperiencesData);
			}
			fwrite($resultfile, PHP_EOL.'saved!'.PHP_EOL);
		}
		...
		fwrite($resultfile, 'checkapplicant'.json_encode($applicant));
		fclose($resultfile);
		return $applicant;
	} 
    ...
    public function updateApplicantCertificates( $applicantId )
    {
        ...
        $certificateData = [
			...
			'certificate_training'              => isset( $certificate['inhouse'] ),
