 ...
 * (c) 2014 Clemente Qui«Ğones Jr. <clemquinones@gmail.com>
 */

/**
 * Core Knowledge of all pages
 *
 * @author     Clemente Qui«Ğones Jr. <clemquinones@gmail.com>
 * @version    1.0.0
 */
...
class m_report extends MY_Model {
	...
	public function getApplicants()
	{
		...
		if ( $status == $statuses['Selected'] ) {
			$this->db->join( 'employer_selected', 'selected_applicant = applicant_id' )
				->where(
					"DATE(selected_date) BETWEEN '".$dateFrom."' AND '".$dateTo."'",
					null, false);
	...
	public function getApplicantStatusReports( $options )
	{
		...
		$recruitmentAgent = $options['recruitmentAgent'];
		...
		if ( (int) $recruitmentAgent > 0 ) {
			$recruitmentAgent = (int) $recruitmentAgent;

			$this->db->where([
				'agent_id' => $recruitmentAgent,
			]);
		}
